...
void attach_virtio(void* mmio_base, int irqno) {
    ...
    extern void viocons_attach(volatile struct virtio_mmio_regs * regs, int irqno);  // viocons.c

    extern void vioblk_attach(volatile struct virtio_mmio_regs * regs, int irqno);  // vioblk.c

    extern void viorng_attach(volatile struct virtio_mmio_regs * regs, int irqno);  // viorng.c

    extern void viogpu_attach(volatile struct virtio_mmio_regs * regs, int irqno);  // viogpu.c

    extern void viohi_attach(volatile struct virtio_mmio_regs * regs, int irqno);  // viohi.c
    ...
    if (regs->device_id == VIRTIO_ID_NONE) return;
    ...
    switch (regs->device_id) {
        ...
        default:
            debug("%p: Unknown virtio device type %u ignored\n", mmio_base,
                  (unsigned int)regs->device_id);
            return;
    }
}

int virtio_negotiate_features(volatile struct virtio_mmio_regs* regs, virtio_featset_t enabled,
                              const virtio_featset_t wanted, const virtio_featset_t needed) {
    uint_fast32_t i;
    ...
    for (i = 0; i < VIRTIO_FEATLEN; i++) {
        if (needed[i] != 0) {
            ...
            if ((regs->device_features & needed[i]) != needed[i]) return -ENOTSUP;
        }
    }
...
void virtio_attach_virtq(volatile struct virtio_mmio_regs* regs, int qid, uint_fast16_t len,
...
void __attribute__((weak)) viocons_attach(volatile struct virtio_mmio_regs* regs, int irqno) {
    // nothing
}

void __attribute__((weak)) vioblk_attach(volatile struct virtio_mmio_regs* regs, int irqno) {
    // nothing
}

void __attribute__((weak)) viorng_attach(volatile struct virtio_mmio_regs* regs, int irqno) {
    // nothing
}

void __attribute__((weak)) viogpu_attach(volatile struct virtio_mmio_regs* regs, int irqno) {
    // nothing
}

void __attribute__((weak)) viohi_attach(volatile struct virtio_mmio_regs* regs, int irqno) {
    // nothing
}