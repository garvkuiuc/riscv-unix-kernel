...
smode_trap_entry_from_umode:
        ...
        # save state into the U trap frame
        sd      a0, A0(sp)
        sd      a1, A1(sp)
        sd      a2, A2(sp)
        sd      a3, A3(sp)
        ...
        # U-mode preserve user gp/tp/sp
        sd      gp, GP(sp)
        sd      tp, TP(sp)
        csrr    t6, sscratch
        sd      t6, SP(sp)

        # switch sscratch to  S-mode
        csrw    sscratch, zero

        # Save minimal CSRs/context needed for return
        rdinstret t6
        sd      t6, SINSTRET(sp)
        ...
        # Allow S-mode to access validated user pages during trap
        li      t6, (1 << 18)          # RISCV_SSTATUS_SUM
        csrs    sstatus, t6

        csrr    t6, sepc
        sd      t6, SEPC(sp)

        # Establish kernel frame and restore kernel gp/tp
        addi    fp, sp, TFRSZ
        ...
        # Handle exception/interrupt in C and return here
        call    smode_trap_entry_from_umode_cont

        # Restore registers in a safe order for U-mode return
        ld      a0, A0(sp)
        ld      a1, A1(sp)
        ld      a2, A2(sp)
        ld      a3, A3(sp)
        ...
        # Restore status with interrupts disabled 
        ld      t6, SSTATUS(sp)
        andi    t6, t6, ~2
        csrw    sstatus, t6

        # Provide re-entry trap frame pointer for next trap
        csrw    sscratch,  sp
...
smode_trap_entry_from_umode_cont:
        ...
        # MSB=1 = interrupt clear MSB before dispatch
        slli    a0, a0, 1
        srli    a0, a0, 1
