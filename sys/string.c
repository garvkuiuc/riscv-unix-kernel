...
static size_t format_int(void (*putcfn)(char, void *), void *aux, uintmax_t val, unsigned int base,
                         int zpad, unsigned int len);

static size_t format_str(void (*putcfn)(char, void *), void *aux, const char *s, unsigned int len);

size_t format_char(void (*putcfn)(char, void *), void *aux, int c, unsigned int len);
...
unsigned long strtoul(const char *str, char **endptr, int base) {
    ...
    // FIXME I don't like this structure at all. It works, but it is ugly.
...
size_t vgprintf(void (*putcfn)(char, void *), void *aux, const char *fmt, va_list ap) {
    const char *p;
    size_t nout;
    int zpad;
    unsigned int len;
    ...
    while (*p != '\0') {
        if (*p == '%') {
            ...
            // Check for l and other prefixes
            ...
            if (*p == 'z') {
                if (sizeof(size_t) == sizeof(int)) lcnt = 0;
                if (sizeof(size_t) == sizeof(long)) lcnt = 1;
                if (sizeof(size_t) == sizeof(long long)) lcnt = 2;
                p += 1;
            } else if (*p == 'j') {
                if (sizeof(intmax_t) == sizeof(int)) lcnt = 0;
                if (sizeof(intmax_t) == sizeof(long)) lcnt = 1;
                if (sizeof(intmax_t) == sizeof(long long)) lcnt = 2;
                p += 1;
            } else if (*p == 'h') {
                p += 1;
            }
...
size_t format_int(void (*putcfn)(char, void *), void *aux, uintmax_t val, unsigned int base,
                  int zpad, unsigned int len) {
    char buf[40];
    size_t nout;
    char *p;
...
size_t format_str(void (*putcfn)(char, void *), void *aux, const char *s, unsigned int len) {
    size_t nout = 0;
