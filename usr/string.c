...
/*!
 * @brief Put a chracter c into buffer.
 * @param c The character to put into buffer.
 * @param aux Auxiliary pointer for managing the buffer.
 */
static void vsnprintf_putc(char c, void * aux);

/*!
 * @brief Convert an integer val into string given the base, padding and minimum length. 
 * Then use putcfn to put it into buffer.
 * @param putcfn The putc function.
 * @param aux Auxiliary pointer for managing the buffer.
 * @param val The integer value.
 * @param base The base of the integer
 * @param zpad Indicate whether to use zero padding or space padding.
 * @param len Minimum length that will be written into buf, the rest will be padding.
...
/*!
 * @brief Put a given string into a buffer given a putc function and a minimum length.
 * @param putcfn The putc function.
 * @param aux Auxiliary pointer for managing the buffer.
 * @param s The string to be written into the buffer.
 * @param len Minimum length that will be written into the buffer, the rest will be padded with space.
 * @return Number of characters written into buffer.
 */
...
/*!
 * @brief Redirection to putc function for printf
 * @param c The character to write to the main UART
 * @param aux Unused
 * @return None
 */
...
/*!
 * @brief Redirection to dputc function for dprintf
 * @param c The character to write to the file descriptor
 * @param aux Auxiliary pointer containing reference to the file descriptor
 * @return None
 */
...
int strncasecmp(const char * s1, const char * s2, size_t n) {
    ...
    if (n != 0)
      return (unsigned int)toupper(*s1) - (unsigned int)toupper(*s2);
    else
      return 0;
}
...
unsigned long strtoul(const char * str, char ** endptr, int base) {
    ...
    // I don't like this structure at all. It works, but it is ugly.
